...
## A duplicate of the framebuffer generated by [method deferred_refresh] once per
## timer tick.
var finished_frame: BitMap
...
func _ready() -> void:
	...
	clock.timer_pulse.append(deferred_refresh)
...
func refresh() -> void:
	mutex.lock()
	if visible:
		texture.update(finished_frame.convert_to_image())
	mutex.unlock()
	refreshed.emit()
...
## Allows refreshing the display from a thread
func deferred_refresh() -> void:
	mutex.lock()
	finished_frame = framebuffer.duplicate()
	mutex.unlock()
